...
with open("res/" + model.replace('/','-') + "-" + tags_path.split('/')[-1].replace('.tags','') + "-" + str(shots_num) + "shot.jsonl", "w", encoding = "UTF-8") as af:
    for res in all_res:
        af.write(json.dumps(res, ensure_ascii = False) + "\n")

with open("res/" + model.replace('/','-') + "-" + tags_path.split('/')[-1].replace('.tags','') + "-" + str(shots_num) + "shot-other.jsonl", "w", encoding = "UTF-8") as af:
