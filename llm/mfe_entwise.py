...
if shots_num > 0:
    ...
    for t in all_entities.keys():
        all_entities[t] = Counter(all_entities[t]).most_common(shots_num)

    # print(list(all_entities.items()))

    mfe_prompt = "\nHere are most frequent examples for each entity class, separated by comma:\n" + "\n".join([t + ": " + ", ".join([e[0] for e in es]) for t, es in sorted(list(all_entities.items()), key = lambda x : x[0])])
...
with open("res/mfe-entwise-seed=" + str(seed) + "-" + model.replace('/','-') + "-" + tags_path.split('/')[-1].replace('.tags','') + "-" + str(shots_num) + "shot.jsonl", "w", encoding = "UTF-8") as af:
    for res in all_res:
        af.write(json.dumps(res, ensure_ascii = False) + "\n")

with open("res/mfe-entwise-seed=" + str(seed) + "-" + model.replace('/','-') + "-" + tags_path.split('/')[-1].replace('.tags','') + "-" + str(shots_num) + "shot-other.jsonl", "w", encoding = "UTF-8") as af:
