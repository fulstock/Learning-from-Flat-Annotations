    ...
    for f in tqdm(files):
        ...
        if f[-4:] == '.ann':
            try:
                ...
                # innermost
                # entities = [v for v in entities if not len([s for s in entities if v is not s and v[0] <= s[0] and v[1] >= s[1]]) > 0]
                ...
                # entity_start_chars, entity_end_chars, entity_types = [list(l) for l in zip(*entities)]
                # pass 
                spans = []
                
                start_words, end_words = zip(*offset_mapping)

                for i, (s, e, t) in enumerate(entities):

                    doc = {
                        'text' : txtdata[s : e],
                        'entity_types' : [t],
                        'entity_start_chars' : [0],
                        'original_entity_start_chars' : [s],
                        'entity_end_chars' : [e - s],
                        'original_entity_end_chars' : [e],
                        'id' : f[:-4] + "_fe" + str(i),
                        'word_start_chars' : [sw - s for sw, ew in zip(start_words, end_words) if sw >= s and ew <= e],
                        'original_word_start_chars' : [sw for sw, ew in zip(start_words, end_words) if sw >= s and ew <= e],
                        'word_end_chars' : [ew - s for sw, ew in zip(start_words, end_words) if sw >= s and ew <= e],
                        'original_word_end_chars' : [ew for sw, ew in zip(start_words, end_words) if sw >= s and ew <= e],
                    }
                    ...
                    doc_count += 1
                    doc_ids.append(f[:-4] + "_fe" + str(i))

                    entities_count += len(doc["entity_types"])
